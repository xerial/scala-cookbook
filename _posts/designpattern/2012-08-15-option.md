...
if文やパターンマッチを使ったnull(またはNone)の値のチェックは、ログインをするためのコードの本質的な部分ではないので、この部分を気にせずプログラミングできることが好ましいです。そのために、monadのmap, flatMapを使うとコードの流れを妨げずに、必要な処理に的を絞ってコードを書くことができます。
